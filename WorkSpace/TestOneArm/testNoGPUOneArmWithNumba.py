...
space2 = np.zeros([x_size, a1_size], dtype=float)
env = zer
...
def findTargetArea():
    for config in feasible_area:
        if rPresence(config[0], config[1])[target[0], target[1]] == 1:
            ...
            space2[config[0], config[1]] = 1


@jit(nopython=True)
def findPath(space_, feasible_area_):
    space2_ = np.zeros([x_size, a1_size])
    iteration = 0
    while space_[nx0, na10] < 0.00000001:
        iteration += 1
        for config in feasible_area_:
            nx = config[0]
            na1 = config[1]
            delta = 0
            for i in range(3):
                for j in range(3):
                    delta += space_[min(max(nx - 1 + i, 0), 191),
                                    min(max(na1 - 1 + j, 0), 17)]
            space2_[nx, na1] = np.tanh(delta)

            delta = 0
            for i in range(3):
                for j in range(3):
                    delta += space2_[min(max(nx - 1 + i, 0), 191),
                                     min(max(na1 - 1 + j, 0), 17)]
            space_[nx, na1] = np.tanh(delta)
